...
Help() {
 ...
 #!===
  ...
  # @return:      NONE (uses globale temporäre Arrays _temp, _temp_section_order, _temp_menu_order)
  #
      ...
      case "$line" in
        ...
        *=*)
          ...
          if [[ -n "$current_section" && -n "$key" ]]; then
            _temp["$current_section.$key"]="$value"
  ...
  create_auto_menu_from_outputs() {
    ...
    _temp["$auto_menu_name.type"]="menu"

    # --- Add each output section as menu entry ---
    local index=0
    for section in "${_temp_section_order[@]}"; do
        if [[ "$section" != "$auto_menu_name" && "$section" != "meta" && "$section" != "Messages" && "$section" != "Buttons" && "${_temp[$section.type]}" == "output" ]]; then
            local menu_key="item$index"
            _temp["$auto_menu_name.$menu_key"]="$section"
  ...
  validate_ini_structure() {
    ...
    for section in "${_temp_section_order[@]}"; do
        if [[ "${_temp[$section.type]}" == "menu" ]]; then
            has_menu_sections=true
            break
        elif [[ "${_temp[$section.type]}" == "output" ]]; then
  ...
  # @description: Parses INI file with caching and loads configuration into global arrays
  #
  ...
  find_available_languages() {
    ...
    for dir in "${valid_dirs[@]}"; do
      ...
      for file in "$dir"/*.ini; do
        ...
        local lang_code="${_temp[meta.lang_code]:-}"
        local lang_name="${_temp[meta.lang]:-}"
        local name="${_temp[meta.name]:-}"
  ...
  # @return:      NONE
  #
  # @use_by:      setup_variables
  # @use_not_directly: NONE
  #
  # @use:         find_best_language_file, show_error, parse_ini_file, validate
  ...
  # @return:      NONE
  #
  # @use_by:      show_option_error, show_file_error, validate_file_path, validate, scan_help_dirs,
  ...
  # @use_not_directly: NONE
  #
  # @use:         find_available_languages, build_breadcrumb, setup_variables, find_menu_sections
  ...
  # @use_not_directly: NONE
  #
  # @use:         setup_variables, show_error, find_menu_sections, build_breadcrumb,
