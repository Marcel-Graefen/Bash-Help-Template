...
Help() {
  ...
  # Globale temporäre Arrays für INI-Parsing
  declare -A _temp_config
  declare -a _temp_section_order
  declare -A _temp_menu_order
  ...
  # @return:      NONE
  #
  # @use_by:      show_error, show_option_error, show_file_error, load_and_cache_config, find_available_languages, scan_dirs
  ...
  # @return:      NONE
  #
  # @use_by:      show_option_error, show_file_error, validate_file_path, validate_config, scan_dirs,
  #               find_available_languages, load_and_cache_config, load_help_file, show_help_menu
  # @use_not_directly: NONE
  #
  ...
  # @description: Validates INI file structure and creates auto-menu if needed
  #
  # @param:       NONE (uses globale temporäre Arrays)
  # @return:      0 if valid, 1 if invalid
  #
  # @use_by:      load_and_cache_config
  # @use_not_directly: NONE
  #
  # @use:         create_auto_menu_from_outputs, log_message
  ...
  validate_ini_structure() {
    ...
    for section in "${_temp_section_order[@]}"; do
        if [[ "${_temp_config[$section.type]}" == "menu" ]]; then
            has_menu_sections=true
            break
        elif [[ "${_temp_config[$section.type]}" == "output" ]]; then
            has_output_sections=true
        fi
    done

    # --- If no menu sections but output sections exist -> create auto-menu ---
    ...
    if [[ "$has_menu_sections" != true ]]; then
        log_message "WARN" "INI file does not contain any valid menu sections"
        return 1
    fi
 ...
 #!===
  ...
  # @return:      NONE (uses globale temporäre Arrays _temp_config, _temp_section_order, _temp_menu_order)
  #
  # @use_by:      load_and_cache_config, find_available_languages
  # @use_not_directly: NONE
  #
  # @use:         NONE
  ...
  # === CREATE AUTO MENU FROM OUTPUTS ===

  # @description: Creates an automatic menu from output sections when no explicit menu exists
  #
  # @param:       NONE (uses globale temporäre Arrays)
  # @return:      NONE
  #
  # @use_by:      validate_ini_structure
  # @use_not_directly: NONE
  #
  # @use:         log_message
  ...
  create_auto_menu_from_outputs() {
    ...
    for section in "${_temp_section_order[@]}"; do
        if [[ "$section" != "$auto_menu_name" && "$section" != "meta" && "$section" != "Messages" && "$section" != "Buttons" && "${_temp_config[$section.type]}" == "output" ]]; then
            local menu_key="$index"  # Changed to number only
  ...
  # @description: LOAD & CACHE CONFIG configuration into global arrays
  #
  ...
  # @use_not_directly: NONE
  #
  # @use:         parse_ini_to_arrays, validate_ini_structure, log_message
  ...
  load_and_cache_config() {
    ...
    # --- Validate INI structure ---
    if ! validate_ini_structure; then
      return 1
    fi
  ...
  # @description: Discovers and validates available language files based on meta information only
  #
  # @param:       NONE
  # @return:      NONE
  #
  ...
  # @use_not_directly: NONE
  #
  # @use:         show_error, scan_dirs, parse_ini_to_arrays, log_message
  ...
  find_available_languages() {
    ...
    for dir in "${valid_dirs[@]}"; do
      ...
      for file in "$dir"*.ini; do
        ...
        local lang_name="${_temp_config[meta.lang]:-}"
  ...
  # @use_not_directly: NONE
  #
  # @use:         find_best_language_file, show_error, load_and_cache_config , validate_config
    ...
    while [[ $current_index -lt $total_contents ]]; do
      ...
      if [[ $total_contents -eq 1 ]]; then
        ...
        if [[ "$content_type" == "text" ]]; then
          show_text_with_buttons "$page_title" "$content" "$BUTTON_CLOSE" ""
        else
          show_file_with_buttons "$page_title" "$content" "$BUTTON_CLOSE" ""
        fi
